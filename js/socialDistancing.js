...
var options = {series: {
  ...
  var chartSim;    
   ...
   dataC = google.visualization.arrayToDataTable([
          ['Day since outbreak', 'Susceptble', 'Infectious', 'Exposed', 'Removed'],
          [,  0,0,0,0]]);
        
		
   chartSim = new google.visualization.AreaChart(document.getElementById('chartContainerSocialDistancing'));
...
document.getElementById("statsSocialDistancing").innerHTML = "<p style=\"color:#0DD3FE;\"> Susceptible: "+ susceptible +"</p><p style=\"color:#F9BD25;\"> Exposed: " + exposed + 
        ...
        for (let i0 = 0; i0 < this.data.length; i0++){
          ...
          if (i !== i0 && geometric.lineLength([d.pos, d0.pos]) < d.radius + d0.radius && !d.collided && !d0.collided){
            ...
            if (keep < swap) {
              ...
              if((d.color=='#0DD3FE') && (d0.color=='#C20292') &&(d.outline!=30)){//d0 d are the two balls colliding
              ...
              //} 
			  else if((d0.color=='#0DD3FE') && (d.color=='#C20292') && (d0.outline!=30)){//states other scenario; this is dicotomous scenario
                 /*var IorE = Math.random(); 
			   if (IorE>=.16)
			   {d0.color = '#C20292';
			    	susceptible--;
  ...
  const mySimulation1 = (_ => {
    const simulation = new Simulation1;
  ...
  // Draw the simulation
  const wrapper1 = document.getElementById("simulationSocialDistancing");
  const canvas1 = document.createElement("canvas1");
  canvas1.width = mySimulation1.width;
  canvas1.height = mySimulation1.height;
  canvas1.style.background = "white";
  wrapper1.appendChild(canvas1);
  const ctx = canvas1.getContext("2d");
  ctx.fillStyle = "#0DD3FE";
  ...
  function tick(){
    ...
    ctx.clearRect(0, 0, mySimulation1.width, mySimulation1.height);

    // The simulation.tick method advances the simulation one tick
    mySimulation1.tick(time_instant);
    for (let i = 0, l = mySimulation1.data.length; i < l; i++){
      const d = mySimulation1.data[i];
      ctx.beginPath();
